...
function Invoke-RuntimeHostAdapterExpect {
    param(
        [string]$Name,
        [int]$ProcessId,
        [string]$ExpectedDecision
    ...
    Write-Output "START runtime-host-adapter-failure-case Name=$Name Pid=$ProcessId ExpectedDecision=$ExpectedDecision"
    $output = & $memoryRobotCliExe runtime-host-adapter-expect $ProcessId $ExpectedDecision
    $exitCode = $LASTEXITCODE
    foreach ($line in $output) {
        Write-Output $line
    }
    ...
    if ($exitCode -eq 0) {
        Write-Output "OK runtime-host-adapter-failure-case Name=$Name Pid=$ProcessId Decision=$ExpectedDecision"
        return
    }

    Write-Output "FAIL runtime-host-adapter-failure-case Name=$Name Pid=$ProcessId Exit=$exitCode ExpectedDecision=$ExpectedDecision"
    $script:passed = $false
}
...
Invoke-RuntimeHostAdapterExpect -Name "missing-process" -ProcessId 999999 -ExpectedDecision "MemoryNotReady:ProcessUnavailable"
...
if ($null -ne $system) {
    ...
    Invoke-RuntimeHostAdapterExpect -Name "system-access" -ProcessId $system.Id -ExpectedDecision "MemoryNotReady:AccessDenied"
}
else {
    ...
    Write-Output "SKIP runtime-host-adapter-failure-case Name=system-access Reason=`"SystemProcessUnavailable`""
}
...
if ($null -ne $explorer) {
    ...
    Invoke-RuntimeHostAdapterExpect -Name "explorer-bitness" -ProcessId $explorer.Id -ExpectedDecision "MemoryNotReady:TargetNot32Bit"
}
else {
    ...
    Write-Output "SKIP runtime-host-adapter-failure-case Name=explorer-bitness Reason=`"ExplorerUnavailable`""
}
